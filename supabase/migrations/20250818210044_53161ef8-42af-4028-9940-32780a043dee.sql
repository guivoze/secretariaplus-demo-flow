-- Create demo_sessions table for tracking user sessions
CREATE TABLE public.demo_sessions (
  ...
  completion_percentage DECIMAL(5,2) GENERATED ALWAYS AS (
    CASE 
      WHEN total_steps > 0 THEN (current_step::decimal / total_steps::decimal) * 100
      ELSE 0
    END
  ) STORED,
  ...
  lead_score INTEGER GENERATED ALWAYS AS (
    CASE
      ...
      WHEN completion_percentage >= 80 THEN 80
      WHEN completion_percentage >= 60 THEN 60
      WHEN completion_percentage >= 40 THEN 40
      WHEN completion_percentage >= 20 THEN 20
      ELSE 10
    END
  ) STORED,
...
CREATE POLICY "Demo sessions are publicly readable" 
